...
private void performSJFPreemptiveScheduling() {
    int n = processData.size(); // Number of processes
    double avgCompletionTime = 0;
    double avgTurnaroundTime = 0;
    double avgWaitingTime = 0;
    ...
    for (int i = 0; i < n; i++) {
        arrivalTime[i] = Integer.parseInt(processData.get(i)[2]); // Assuming arrival time is at index 2
        burstTime[i] = Integer.parseInt(processData.get(i)[3]);   // Assuming burst time is at index 3
    ...
    while (completedProcesses < n) {
        ...
        if (shortestJobIndex == -1) {
            ...
            // Determine the time slice (minimum burst time) for the process
            int timeSlice = remainingBurstTime[index];
