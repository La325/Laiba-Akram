...
private List<Integer> burstTimesorg = new ArrayList<>();
   ...
   public boolean containsProcessID(String processID) {
       ...
       public List<PHH2> getProcessesFromTable2() {
        List<PHH2> processes = new ArrayList<>();
        DefaultTableModel model = (DefaultTableModel) jTable2.getModel();
        int rowCount = model.getRowCount();

        for (int i = 0; i < rowCount; i++) {
            String processID = (String) model.getValueAt(i, 0);
            String processName = (String) model.getValueAt(i, 1);
            int ArrivalTime=  (int) model.getValueAt(i, 2);
            int BurstTime = (int) model.getValueAt(i, 3);
 ...
 public String getProcessStatus(String processID) {
        System.out.println("Checking process ID: " + processID);
        for (int i = 0; i < model.getRowCount(); i++) {
            System.out.println("Table process ID: " + model.getValueAt(i, 0) + ", Status: " + model.getValueAt(i, 4));
            if (model.getValueAt(i, 0).equals(processID)) {
                return (String) model.getValueAt(i, 4);
            }
        }
        return null;
    }
...
public void ProcessStatus(String status, String priority) {
        for (int i = 0; i < model.getRowCount(); i++) {
            if ("Ready".equals(model.getValueAt(i, 4))) {
                model.setValueAt(status, i, 4); 
                model.setValueAt(priority, i, 5); 
            }
        }
...
//       DefaultTableModel model = (DefaultTableModel) jTable2.getModel();
//        int rowCount = model.getRowCount();
//        boolean processDispatched = false;
//        
//        for (int i = 0; i < rowCount; i++) {
//            String currentState = model.getValueAt(i, 4).toString();  // Assuming the state is in the 5th column (index 4)
//            if (currentState.equals("Ready")) {
//                model.setValueAt("Running", i, 4);  // Update state to "Running"
//                model.setValueAt("High", i, 5);     // Update priority to "High"
//                processDispatched = true;
//            }
//        }
//
//        if (processDispatched) {
//            JOptionPane.showMessageDialog(this, "All ready processes are now RUNNING.");
////            // Assuming you have a class Scheduling with a method schedule
////            Schedulingg scheduling = new Schedulingg();
////            scheduling.setVisible(true);
//        } else {
//            JOptionPane.showMessageDialog(this, "No ready processes to dispatch.");
//        }
